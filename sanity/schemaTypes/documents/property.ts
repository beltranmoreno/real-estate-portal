...
/**
 * Validation helper: field is required only when the property is Active.
 * Lets us save "pending" stubs with just a title/photos so the agent can
 * send the owner a completion link before all fields are filled in.
 */
const requiredWhenActive = (Rule: any) =>
  Rule.custom((value: any, context: any) => {
    const status = context.document?.status
    if (status !== 'active') return true
    if (value === undefined || value === null || value === '') {
      return 'Required before property can be set to Active'
    }
    return true
  })
...
export const property = defineType({
  ...
  groups: [
    ...
    {name: 'completion', title: 'Owner Completion'},
    {name: 'seo', title: 'SEO'},
  ],
  fields: [
    // Basic Info
            ...
            },
            {
              ...
              options: {
                list: [
                  ...
                  {title: 'Game Room', value: 'game-room'},
                  {title: 'BBQ', value: 'bbq'},
    ...
    // Owner Completion Flow
    // These fields power the "send a link to the owner" workflow.
    // The token is generated by the Studio action and verified by the
    // public completion endpoints. Never exposed through read APIs to
    ...
    defineField({
      ...
      group: 'completion',
      readOnly: true,
      description: 'Random token that authorizes the owner to complete this listing. Regenerate to revoke any old link.',
    }),
    defineField({
      name: 'completionTokenExpiresAt',
      title: 'Completion Token Expires At',
      type: 'datetime',
      group: 'completion',
    }),
    defineField({
      name: 'completionDraft',
      title: 'Owner Draft (auto-saved)',
      type: 'object',
      group: 'completion',
      readOnly: true,
      description: 'In-progress form state saved by the owner. Cleared when they submit the final version.',
      fields: [
        {
          name: 'data',
          title: 'Draft JSON',
    ...
    defineField({
      ...
      group: 'completion',
      readOnly: true,
      description: 'Who submitted the completion form, if anyone has.',
      fields: [
